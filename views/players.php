...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        !empty($name_player) && !empty($photo_player) && !empty($nationality_player) && !empty($club_player)
        && !empty($position_player) && !empty($rating_player)
    ) {

        if ($position_player === 'GK') {
            if (
                !empty($diving_GK) && !empty($handling_GK) && !empty($kicking_GK) &&
                !empty($reflexes_GK) && !empty($speed_GK) && !empty($positioning_GK)
            ) {

                $stmtStatsGK = $connection->prepare("INSERT INTO StatistiqueGKs (diving, handling, kicking, reflexes, speed, positioning) VALUES (?, ?, ?, ?, ?, ?)");
                $stmtStatsGK->bind_param('iiiiii', $diving_GK, $handling_GK, $kicking_GK, $reflexes_GK, $speed_GK, $positioning_GK);
                $stmtStatsGK->execute();
                $statistgk_id = $connection->insert_id;

                $stmt = $connection->prepare("INSERT INTO Players (name_player, photo, nationality_id, club_id, position, rating, statistgk_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmtStatsGK->close();
            } else {
                echo "<script>alert('Veuillez remplir toutes les statistiques de gardien de but.');</script>";
                return;
            }
        } else {
            if (
                !empty($pace_player) && !empty($shooting_player) && !empty($passing_player) &&
                !empty($dribbling_player) && !empty($defending_player) && !empty($physical_player)
            ) {
                ...
                echo "<script>alert('Veuillez remplir toutes les statistiques de joueur.');</script>";
                return;
            }
        }
        ...
        if ($position_player === 'GK') {
            $stmt->bind_param('ssssssi', $name_player, $photo_player, $nationality_id, $club_id, $position_player, $rating_player, $statistgk_id);
        } else {
...
$sql = "SELECT p.player_id, p.name_player, p.photo, n.nationality_name, n.flag, c.club_name,c.logo, p.position, p.rating, s.pace, s.shooting, s.passing, s.dribbling, s.defending,  s.physical,g.diving, g.handling, g.kicking, g.reflexes, g.speed, g.positioning
        FROM Players p
        LEFT JOIN Nationalities n ON p.nationality_id = n.nationality_id
        LEFT JOIN Clubs c ON p.club_id = c.club_id
        LEFT JOIN StatistiqueJrs s ON p.statistjr_id = s.statistjr_id
        LEFT JOIN StatistiqueGKs g ON p.statistgk_id = g.statistgk_id;";
...
<body>
    <nav>
        <div class="logo-name">
            ...
            <span class="logo_name">FUT Ultimate</span>
        </div>
        <div class="menu-items">
            <ul class="nav-links">
                <li><a href="../index.php"><i class="fas fa-tachometer-alt"></i><span class="link-name">Dashboard</span></a></li>
                <li><a href="players.php"><i class="fas fa-users"></i><span class="link-name">Players</span></a></li>
                <li><a href="/FUT-Champions-Content-Manager/views/nationality.php"><i class="fas fa-flag"></i><span class="link-name">Nationality</span></a></li>
                <li><a href="/FUT-Champions-Content-Manager/views/club.php"><i class="fas fa-building"></i><span class="link-name">Club</span></a></li>
                <li><a href="#"><i class="fas fa-dumbbell"></i><span class="link-name">Stats JR</span></a></li>
                <li><a href="#"><i class="fas fa-fist-raised"></i><span class="link-name">Stats GK</span></a></li>
            </ul>
            <ul class="logout-mode">
                <li><a href="#"><i class="fas fa-sign-out-alt"></i><span class="link-name">Logout</span></a></li>
                <li class="mode"><a href="#"><i class="fas fa-moon"></i><span class="link-name">Dark Mode</span></a></li>
            </ul>
        </div>
    </nav>
                                            ...
                                            while ($row = $result->fetch_assoc()) : ?>
                                                <tr>
                                                    ...
                                                    <?php if ($row["position"] === 'GK'): ?>
                                                        <td class="px-4 py-3"><?= htmlspecialchars($row["diving"]) ?></td>
                                                        <td class="px-4 py-3"><?= htmlspecialchars($row["handling"]) ?></td>
                                                        <td class="px-4 py-3"><?= htmlspecialchars($row["kicking"]) ?></td>
                                                        <td class="px-4 py-3"><?= htmlspecialchars($row["reflexes"]) ?></td>
                                                        <td class="px-4 py-3"><?= htmlspecialchars($row["speed"]) ?></td>
                                                        <td class="px-4 py-3"><?= htmlspecialchars($row["positioning"]) ?></td>
                                                    <?php else: ?>
    ...
    <script>
        ...
        const cancelButtonPlayer = document.getElementById("cancelButtonPlayer");
        cancelButtonPlayer.addEventListener("click", () => {
        formulairePlayer.classList.toggle("hidden");
        });
    </script>
