...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $query = "SELECT * FROM Nationalities WHERE nationality_id = ?";
...
if (isset($_POST['updateNationalite'])) {
    $nationality_name = mysqli_real_escape_string($connection, $_POST['nationality_name']);
    $flag_url = mysqli_real_escape_string($connection, $_POST['flag_url']);
    $id = $_POST['id'];

    $sql = "UPDATE Nationalities SET nationality_name = ?, flag = ? WHERE nationality_id = ?";
    $stmt = mysqli_prepare($connection, $sql);
    mysqli_stmt_bind_param($stmt, 'ssi', $nationality_name, $flag_url, $id);
        ...
        <form id="nationalityForm" class="max-w-sm mx-auto mt-5" method="POST" action="">
            <div class="mb-5">
                ...
                <input type="text" name="nationality_name" id="nationality_name" 
                       ...
                       required value="<?php echo htmlspecialchars($row['nationality_name']); ?>">
            </div>
            <div class="mb-5">
                ...
                <input type="url" name="flag_url" id="flag_url" 
                       class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg w-full p-2.5" 
                       required value="<?php echo htmlspecialchars($row['flag']); ?>">
            </div>
            <input type="hidden" name="id" value="<?php echo $row['nationality_id']; ?>">
            <button type="submit" class="bg-blue-700 text-white px-5 py-2 rounded-lg" name="updateNationalite" value="UPDATE">Enregistrer</button>
        </form>
    </section>
</body>
</html>
